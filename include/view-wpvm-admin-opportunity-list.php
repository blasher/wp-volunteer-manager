...
if (!class_exists("WPVM_View_Admin_Opportunity_List"))
{	class WPVM_View_Admin_Opportunity_List extends WPVM_View
	{
		function display()
		{
			require_once( WPVM_PLUGIN_PATH . '/include/class-wpvm-admin-opportunity-table.php' );

			self::wpvm_button_add_new();
			echo '<div id="wpvm_opportunity_list_table" style="margin-right:15px;">';
			self::wpvm_opportunities_table();
			echo '</div>';
		}
		...
		{	
			...
			$out .= '<form method="post" id="Opportunity_Action_Form" name="Opportunity_Action_Form" action="" />';
			$out .= '<input type="hidden" id="wpvm_id" name="id" value="" />';
			$out .= '<input type="hidden" id="wpvm_action" name="action" value="new" />';
			$out .= '<input type="submit" value="Add New Volunteering Opportunity" />';
			$out .= '</div>';
			$out .= '</form>';
		...
		function wpvm_opportunities_table()
		{
			require_once( ABSPATH . '/wp-admin/includes/class-wp-list-table.php' );

			//Prepare Table of elements
			$wp_list_table = new WPVM_Opportunity_List_Table();
